...
// precisar verificar domínio — funciona bem pra mandar pro próprio e-mail da conta. Se um domínio
// verificado for configurado no Resend depois, é só preencher RESEND_FROM_EMAIL no .env.
...
/**
 * Manda um e-mail com anexos (CSV) via Resend. Nunca lança erro — só registra no log — porque isso
 * roda dentro do cron de publicar-mencoes (ver exportarRelatoriosDevidos em relatorios.ts): uma
 * falha de e-mail não pode derrubar a publicação de Stories nem impedir o CSV de ser salvo no
 * Storage, que já aconteceu antes desta chamada.
 */
    ...
    const resposta = await fetch(RESEND_API_URL, {
      ...
      body: JSON.stringify({
        ...
        attachments: anexos.map((a) => ({
          ...
          // Resend espera o conteúdo do anexo em base64.
          content: Buffer.from(a.conteudoCSV, "utf-8").toString("base64"),
        })),
      }),
    });
